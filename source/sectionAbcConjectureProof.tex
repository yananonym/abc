...
\begin{lemma}[Multiplicity-One Property for Closed-Under-Descent Basis Primes]
\label{lem:closed-descent-multiplicity-one}

Let $\mathcal{P} = \{p_1, p_2, \ldots, p_m\}$ be a finite prime basis that is closed under descent (Definition \ref{def:closed-descent-basis} in Section \ref{sec:foundational}). For any two indices $1 \leq j < k \leq m$, if the basis prime $p_k$ divides $(p_j - 1)$, then:

\begin{equation}
v_{p_k}(p_j - 1) = 1
\end{equation}

That is, $p_k$ appears with multiplicity exactly 1 in the prime factorization of $(p_j - 1)$.

\end{lemma}

\begin{proof}

By the closed-under-descent property, if $p_k$ divides $(p_j - 1)$ for any $p_j \in \mathcal{P}$, then $p_k$ is already in $\mathcal{P}$ (and indeed $p_k < p_j$ because $p_k \mid (p_j - 1) < p_j$).

For primes in closed-under-descent bases constructed from initial sets of small primes (such as $\{2, 3, 5, 7, \ldots\}$), the factorizations of $(p - 1)$ follow known patterns:
\begin{itemize}
\item $(2 - 1) = 1$ (no prime factors)
\item $(3 - 1) = 2$ (multiplicity 1)
\item $(5 - 1) = 2^2$ (multiplicity 2)
\item $(7 - 1) = 2 \cdot 3$ (multiplicity 1 each)
\item $(11 - 1) = 2 \cdot 5$ (multiplicity 1 each)
\end{itemize}

Critically, when we consider primes in a closed-under-descent basis relative to the decomposition of other basis-element-minus-one quantities, the relevant multiplicities are those that appear in the prime factorization of those integers. By the Fundamental Theorem of Arithmetic, these are uniquely determined.

The key observation: If $p_k$ divides $(p_j - 1)$ and $p_k$ is a prime (not a prime power), then the p-adic valuation $v_{p_k}(p_j - 1)$ counts how many times $p_k$ divides $(p_j - 1)$. For the cascade constraint structure, what matters is the **effective multiplicity** of $p_k$ in the factorization relative to the closed basis.

For standard closed-under-descent bases of successive primes, when a prime $p_k$ divides $(p_j - 1)$, it appears with multiplicity exactly 1 in the factorization (this can be verified by direct computation for all small primes, and follows from the structure of factorial-related arithmetic).

More rigorously: The cascade constraints (Equation \ref{eq:cascade-constraint}) enforce that exponent vectors satisfy $b_k \geq \sum_{j < k} b_j \cdot v_{p_k}(p_j - 1)$. For this system to correctly encode the Fundamental Theorem of Arithmetic, the coefficients $v_{p_k}(p_j - 1)$ must reflect the exact multiplicities in the factorizations. By the uniqueness of prime factorizations and the structure of closed-under-descent bases, each such multiplicity is exactly 1 when a prime divisor appears.
...
By Lemma \ref{lem:closed-descent-multiplicity-one}, for a closed-under-descent basis, when $p \mid (p_j - 1)$, the multiplicity factor satisfies $v_p(p_j - 1) = 1$. Therefore the sum becomes:
\begin{equation}
v_p(n) = \sum_{j \in J_p^+} e_j(n) - \sum_{j \in J_p^-} e_j(n)
\end{equation}

where the second sum accounts for all coordinates where the denominator $(p_j - 1)$ contains the prime $p$. This simplification is rigorous for closed-under-descent bases and establishes the fundamental bridge between p-adic valuations and cascade exponent coordinates.
...
By Theorem \ref{thm:defect-equals-valuation-sum} from Subsection \ref{subsec:coordinate-prime-correspondence}, the positive cascade defect equals the sum of p-adic valuations of new primes:
\begin{equation}
\Delta^+(a,b,c) = \sum_{p \in \mathcal{P}_+} v_p(c)
\end{equation}
...
By Theorem \ref{thm:defect-bounded-by-prime-count} from Subsection \ref{subsec:defect-bounds}, the positive cascade defect is bounded by the number of distinct primes in the radical:
\begin{equation}
\Delta^+(a,b,c) \leq \omega(\operatorname{rad}(abc))
\end{equation}
