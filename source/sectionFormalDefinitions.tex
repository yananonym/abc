...
\ENSURE The epimoric encoding $E(n) = (e_1, e_2, \ldots, e_m)$ such that $n = \prod_{k=1}^m \left(\frac{k+1}{k}\right)^{e_k}$
...
\STATE \textbf{Step 2:} For each epimoric ratio index $k = 1, 2, 3, \ldots$ in increasing order:
\FOR{$k = 1$ to $m_{\max}$}
  \STATE \textbf{Step 2a:} Compute the prime factorization of the ratio $\frac{k+1}{k}$:
  \STATE \quad $\frac{k+1}{k} = \prod_p p^{w_p(k)}$ where $w_p(k) := v_p(k+1) - v_p(k)$

  \STATE \textbf{Step 2b:} For each prime $p$, check the constraint:
  \FOR{each prime $p$ dividing $n$}
    \STATE Compute $w_p(k) = v_p(k+1) - v_p(k)$ using Legendre's formula or direct prime factorization
  \ENDFOR

  \STATE \textbf{Step 2c:} Greedily assign the maximum exponent $e_k$ such that the denominators don't exceed the numerator:
  \STATE For each prime $p$, the contribution from $e_1, \ldots, e_k$ to the denominator is:
  \begin{equation}
  D_p(e_1, \ldots, e_k) := \sum_{j=1}^k e_j \cdot v_p(j) \quad \text{(denominator exponent at } p \text{)}
  \end{equation}
  \STATE The numerator exponent at $p$ is $a_p = v_p(n)$.
  \STATE The constraint is: for all primes $p$, we need $a_p \geq D_p(e_1, \ldots, e_k)$.

  \STATE \textbf{Step 2d:} Find the maximum valid $e_k$ such that for all primes $p$ with $w_p(k) > 0$:
  \begin{equation}
  e_k \leq \left\lfloor \frac{a_p - D_p(e_1, \ldots, e_{k-1})}{w_p(k)} \right\rfloor
  \end{equation}
  \STATE Set $e_k$ to the minimum across all constraints (the bottleneck prime).

  \STATE \textbf{Step 2e:} If $e_k = 0$ and we have exhausted all primes (i.e., $D_p(e_1, \ldots, e_k) = a_p$ for all $p$), terminate the loop.

  \STATE \textbf{Step 2f:} Update denominators: for each prime $p$,
  \begin{equation}
  D_p(e_1, \ldots, e_k) \gets D_p(e_1, \ldots, e_{k-1}) + e_k \cdot w_p(k)
  \end{equation}
\ENDFOR
