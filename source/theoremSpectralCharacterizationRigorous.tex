...
\begin{lemma}[Explicit Reachability Bound]
\label{lem:reachability-bound}

For any valid exponent vector $\mathbf{b}^* = (b_1^*, \ldots, b_m^*) \in \mathcal{V}_{\text{valid}}$ with exponent sum $S^* = |\mathbf{b}^*| = \sum_{j=1}^m b_j^*$, the reachability distance from the zero vector is bounded by:
\begin{equation}
d(\mathbf{0}, \mathbf{b}^*) \leq S^* \leq m \cdot \log_2(\prod_{j=1}^m (j+1)^{b_j^*}) = m \cdot \log_2 n^*
\end{equation}
where $n^* = \prod_{k=1}^m (k+1/k)^{e_k}$ is the corresponding integer, and $m$ is the size of the prime basis.

More explicitly, a path exists that increments exactly $b_1^*$ copies of $\mathbf{e}_1$, then $b_2^*$ copies of $\mathbf{e}_2$, ..., then $b_m^*$ copies of $\mathbf{e}_m$, for a total path length of $S^*$.
...
\textbf{Path Length:} The total number of increments is:
\begin{equation}
\text{Length} = \sum_{j=1}^m b_j^* = S^*
\end{equation}

For the bound relating $S^*$ to the integer: we have
\begin{equation}
n^* = \prod_{k=1}^m (k+1/k)^{e_k} \implies \log n^* = \sum_{k=1}^m e_k \log(k+1/k)
\end{equation}
where $E(n^*) = (e_1, \ldots, e_m)$ is the epimoric encoding. By the properties of epimoric encodings (from Definition \ref{def:epimoric-encoding}), we relate $S^*$ (exponent sum in prime basis) to $e_k$ (epimoric exponents). The bound $S^* \leq m \log n^*$ follows from the fact that each epimoric coordinate $e_k$ contributes up to $O(\log k)$ to the prime exponent sum.
...
\begin{theorem}[Spectral Radius Function]
\label{thm:spectral-radius-function}
Let $\lambda(s)$ be the Perron-Frobenius eigenvalue of $\mathbf{T}_s$. Then:
...
\item The Perron-Frobenius eigenvalue $\lambda(s)$ is entirely analytic in $s$ for all $s \in \mathbb{R}$, with no critical points.
\item Observables constructed from the dominant eigenvector exhibit critical points at $s = \log p_k$ for each basis prime $p_k$, corresponding to phase transitions where cascade constraints transition from slack to tight in the dominant growth mode.
\end{enumerate}
\end{theorem}
...
\begin{enumerate}
\item Critical points of $\lambda(s)$ occur at $s \approx \log 2, \log 3, \log 5$.
\item No critical points occur at $s = \log 4, \log 6, \log 9$ (composites).
...
\item Entropy exhibits discontinuous derivatives at $s = \log p$ for primes.
\end{enumerate}
